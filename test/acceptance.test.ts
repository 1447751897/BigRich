 ...
 * (① 完整一局闭环见 e2e.test.ts;② 回合上限收束见 core.test.ts;时长硬上限见 timelimit.test.ts;
 *  快照落盘/重启恢复见 snapshot.test.ts;§4B 交易/拍卖/抵押见各自用例。)
 */
...
describe("验收:断线重连 + 网关防作弊(真实 WS)", () => {
  it("重连后资产/进度不丢且连接恢复;他人不能替你行动;客户端伪造骰子被忽略", async () => {
    // 注入内存 store,避免默认 FileSnapshotStore 写盘并跨测试/跨运行污染。
